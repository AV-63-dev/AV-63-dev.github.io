...
class Game {
    ...
    renderSettingsParticipants = () => {
        ...
        dataParticipants.forEach(({ id, name, img }) => {
            this.$participants.insertAdjacentHTML('beforeend', `
                <div class="participant${idPlayer == id ? ' participant_active' : ''}">
                    <img class="participant_img" data-id="${id}" src="${img}" alt="photo of participant ${name}">
                </div>
            `);
        });
    }
    ...
    renderParticipants = () => {
        ...
        players.forEach(({ player, img }) => {
            this.$participants.insertAdjacentHTML('beforeend', `
                <div class="participant${player == "player1" ? ' participant_active' : ' participant_opponent'}">
                    <img src="${img}" alt="participant" style="height: 100%;">
                </div>
            `);
        });
        dataParticipants.forEach(({ img }) => {
            this.$participants.insertAdjacentHTML('beforeend', `
                <div class="participant participant_waiting">
                    <img src="${img}" alt="participant waiting" style="height: 100%;">
                </div>
            `);
        });
        losers.forEach(({ img }) => {
            this.$participants.insertAdjacentHTML('beforeend', `
                <div class="participant participant_losers" style="background-image: url(${img});">
                    <img src="./img/delete.png" alt="participant delete" style="height: 100%;">
                </div>
            `);
        });
    }
